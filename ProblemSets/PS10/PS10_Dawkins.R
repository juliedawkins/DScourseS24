...
load("/Users/juliedawkins/Desktop/PS10.RData")
library(tidyverse)
library(tidymodels)
library(magrittr)
...
rec_wf <- workflow() %>%
  ...
  add_formula(high.earner ~ education + marital.status + race + 
                workclass + occupation + relationship + sex + 
                age + capital.gain + capital.loss + hours)
...
knn_ans %<>% left_join(knn_test %>% slice(1),by=c(".metric",".estimator")) %>%
  mutate(alg = "k nearest neighbors") %>% select(-starts_with(".config"))
